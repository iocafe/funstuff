    ...
    window_y = 90, window_w = 80+5.3, window_h = 120+5.3)
{
    ...
    difference() {
        ...
        if (!is_upper_level) {
           n_holes = 5;
            for (x = [0: n_holes-1]) {
                translate([(x-(n_holes-1)/2) 
                    * ((length-4*wall_thickness)/(n_holes-1)),
                    -width/2 + wall_thickness/2, 
                    wall_height/20]) 
                color(wall_color,wall_alpha) 
                cube([hole_size, 1+wall_thickness, hole_size],
                    center=true);
            }
        }
        ...
        if (is_upper_level) {
            ...
            cube([door_width, wall_thickness+1, 
                door_height],center=true);
        }
    }
