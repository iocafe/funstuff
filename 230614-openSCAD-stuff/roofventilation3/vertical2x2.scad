module vertical2x2(length=100, wood_width = 4, roof_angle=20, slice=0,slice2=0, slice3=0) 
{
    ...
    difference() 
    {
        ...
        if (slice > 0.001) {
            ...
            rotate([0,0,45])
            cube([slice_d, slice_d, length + cut_depth + 2.1], center=true); 
        }
        ...
        if (slice2 > 0.001) {
            ...
            rotate([0,0,45])
            cube([slice_d2, slice_d2, length + cut_depth + 2.1], center=true); 
        }
        ...
        if (slice3 > 0.001) {
            ...
            translate([wood_width/2, 0, -0.5 *(length+cut_depth)+0.05])
            rotate([0,0,45])
            cube([slice_d3, slice_d3, length + cut_depth + 2.1], center=true); 
        }
    }
}
